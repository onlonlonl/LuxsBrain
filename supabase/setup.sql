...
-- Archive recall: search archive + silently trigger hebbian connections
CREATE OR REPLACE FUNCTION archive_recall(keyword TEXT, ctx_size INT DEFAULT 20)
RETURNS TABLE(
...
BEGIN
  PERFORM * FROM hebbian_search(keyword);
  
  RETURN QUERY
  WITH hits AS (
    SELECT a.session_date, a.line_order
    FROM archive a
    WHERE a.content ILIKE '%' || keyword || '%'
    ORDER BY a.session_date, a.line_order
    LIMIT 10
  )
  SELECT DISTINCT a.session_date, a.line_order, a.speaker, a.content, a."timestamp"
  FROM hits h
  JOIN archive a ON a.session_date = h.session_date
    AND a.line_order BETWEEN h.line_order - ctx_size AND h.line_order + ctx_size
  ORDER BY a.session_date, a.line_order;
END;
$$ LANGUAGE plpgsql;
